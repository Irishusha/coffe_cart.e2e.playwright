class BankAccount {
    ...
    public showPin(pin: string): string | null {
      if (pin.length < 4) {
        throw new Error("PIN повинен містити щонайменше 4 символи.");
      }
      const half = Math.floor(pin.length / 2);
      return pin.slice(0, half) + '*'.repeat(pin.length - half);
    }
...
console.log(myAccount.showPin('145787'));